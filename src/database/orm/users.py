...
from src.database.models import User, UserStatus, Wife, user_wife_association
...
async def add_balance(user_id: int, add_to: Decimal):
    async with async_session() as session:
        stmt = select(User).where(User.user_id == user_id)
        result = await session.execute(stmt)
        user = result.scalar_one_or_none()

        user.balance = Decimal(f"{user.balance}") + add_to

        session.add(user)
        await session.commit()


async def remove_balance(user_id: int, add_to: int):
    async with async_session() as session:
        stmt = select(User).where(User.user_id == user_id)
        result = await session.execute(stmt)
        user = result.scalar_one_or_none()
